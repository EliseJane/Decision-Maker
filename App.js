...
export default class App extends Component<Props> {
  ...
  populateInitialData = (tx) => {
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS Options( '
        ...
        + 'question_id INTEGER, '
        + 'FOREIGN KEY ( question_id ) REFERENCES Questions ( id ) ); ').catch((error) => {
        this.errorCB(error)
    });
  ...
  // queryOptions = (tx) => {
  //   tx.executeSql('SELECT option, question_id FROM Options;').then(([tx,results]) => {
  //     const len = results.rows.length;
  //     for (let i = 0; i < len; i++) {
  //         let row = results.rows.item(i);
  //         console.log(`Option: ${row.option}, Question id: ${row.question_id}`)
  //     }
  //   }).catch((error) => {
  //       console.log(error);
  //   });
